...
const { uid, nowTs, mapTauriModel, mapTauriProcess, inferFamily, persistHibernatedState, restoreHibernatedState, persistLastActivity } = await vi.importActual<{
  uid: (prefix?: string) => string;
  nowTs: () => number;
  mapTauriModel: (m: any, workspaceId: string) => any;
  mapTauriProcess: (p: any, models: any[]) => any;
  inferFamily: (name: string, arch: string) => string;
  persistHibernatedState: (ids: string[], instances: any[], ts: number) => void;
  restoreHibernatedState: () => any;
...
describe("lib/helpers", () => {
  ...
  describe("mapTauriModel", () => {
    ...
    it("maps all required fields", () => {
      ...
      expect(model.id).toBe("m123");
      expect(model.name).toBe("Llama-3.1-8B-Instruct-Q4_K_M");
      expect(model.family).toBe("llama3");
      expect(model.sizeGb).toBeCloseTo(7.45, 1); // 8GB / 1.074
      expect(model.quant).toBe("Q4_K_M");
      expect(model.downloaded).toBe(true);
      expect(model.missing).toBe(false);
      expect(model.path).toBe("/models/llama-3.1-8b-q4_k_m.gguf");
      expect(model.hfRepo).toBeUndefined();
      expect(model.builder).toBe("meta-llama");
      expect(model.architecture).toBe("llama");
      expect(model.contextLength).toBe(8192);
      expect(model.parameterCount).toBe("8B");
      expect(model.quantizationBits).toBe(4);
      expect(model.license).toBe("Llama 3.1 Community");
      expect(model.description).toBe("Meta Llama 3.1 8B Instruct");
      expect(model.uploadedAt).toBe("2023-11-14");
      expect(model.hfDownloads).toBe(100000);
      expect(model.tags).toEqual(["instruct", "chat"]);
      expect(model.isMoe).toBe(false);
      expect(model.expertCount).toBeUndefined();
      expect(model.workspaceId).toBe("ws1");
      expect(model.addedAt).toBe(1700000000000);
    });
    ...
    it("detects MoE models from name/architecture", () => {
      ...
      expect(model.expertCount).toBe(8);
    });
    ...
    it("handles null metadata gracefully", () => {
      const modelInfo = { ...mockModelInfo, metadata: null as any };
      const model = mapTauriModel(modelInfo, "ws1");
      expect(model.license).toBe("Unknown");
      expect(model.description).toBe(modelInfo.name);
      expect(model.hfDownloads).toBe(0);
    ...
    it("handles missing architecture/quantization", () => {
      ...
      const model = mapTauriModel(modelInfo, "ws1");
      expect(model.architecture).toBe("llama");
      expect(model.quant).toBe("unknown");
      expect(model.quantizationBits).toBe(4);
    });
  });
  ...
  describe("mapTauriProcess", () => {
    const mockModels = [
      { id: "m1", name: "TestModel", family: "llama", sizeGb: 8, quant: "Q4_K_M", downloaded: true, missing: false, path: "/m.gguf", hfRepo: "org/model", builder: "org", architecture: "llama", contextLength: 8192, parameterCount: "7B", quantizationBits: 4, license: "MIT", description: "", uploadedAt: "", hfDownloads: 0, tags: [], isMoe: false, workspaceId: "ws1", addedAt: 1 },
    ];
    ...
    it("maps process info with matching model", () => {
      ...
      const instance = mapTauriProcess(processInfo, mockModels);
    ...
    it("maps status correctly", () => {
      ...
      for (const [input, expected] of statuses) {
        const processInfo = { id: "p1", model_id: "m1", pid: 1, port: 8080, status: input, started_at: 0, gpu_memory: 0, cpu_memory: 0, context_used: 0, tokens_per_sec: 0 };
        const instance = mapTauriProcess(processInfo, mockModels);
    ...
    it("handles missing model gracefully", () => {
      const processInfo = { id: "p1", model_id: "missing", pid: 1, port: 8080, status: "running", started_at: 0, gpu_memory: 0, cpu_memory: 0, context_used: 0, tokens_per_sec: 0 };
  ...
  describe("Hibernation helpers", () => {
    const mockInstances = [
      { id: "i1", hibernatedConfig: { name: "inst1", model: "m1", profile: "p1", port: 8080, host: "127.0.0.1", gpu: "auto" } },
      { id: "i2", hibernatedConfig: { name: "inst2", model: "m2", profile: "p2", port: 8081, host: "127.0.0.1", gpu: "auto" } },
      { id: "i3", hibernatedConfig: undefined },
    ];

    it("persists hibernated state to localStorage", () => {
      persistHibernatedState(["i1", "i2"], mockInstances as any, 1234567890);
      expect(localStorageMock.setItem).toHaveBeenCalledWith(
        "llama-launcher-hibernation",
        expect.stringContaining("i1")
      );
      expect(localStorageMock.setItem).toHaveBeenCalledWith(
        "llama-launcher-last-activity",
        "1234567890"
      );
    });

    it("persists hibernated state", () => {
      persistHibernatedState(["i1", "i2", "i3"], mockInstances as any, 1234567890);
    ...
    it("restores hibernated state from localStorage", () => {
      const state = {
        ...
        hibernatedConfigs: { i1: { name: "inst1", model: "m1", profile: "p1", port: 8080, host: "127.0.0.1", gpu: "auto" } },
