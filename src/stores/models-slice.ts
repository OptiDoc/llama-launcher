...
export function createModelsSlice(
  ...
  return {
    ...
    importLocalModel: async (options) => {
      ...
      for (const srcPath of paths) {
        try {
          ...
          log.error("[STORE] importLocalModel: failed to import", {
            category: "store",
            context: { path: srcPath, error: String(e) },
          });
        }
      }
      if (imported > 0) {
    ...
    refreshExternalModels: async () => {
      ...
      log.success("[STORE] External models refreshed", {
        category: "store",
        context: { count: externalModels.length },
      });
    },
  };
}
