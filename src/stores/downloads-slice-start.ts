  ...
  return {
    startHFDownload: ({
      ...
      const placeholderModel: LlamaModel = {
        ...
        downloaded: false,
        missing: false,
        path: `${get().globalSettings.modelsDir}/${fallbackFilename}`,
        hfRepo: repo,
        builder,
        architecture: "llama",
        contextLength: 8192,
      ...
      (async () => {
        ...
        try {
          ...
          if (!checkResp.ok) {
            const statusText =
              checkResp.status === 401
                ? "requires authentication (gated repo)"
        ...
        try {
          ...
          if (resp.ok) {
            ...
            const ggufFiles = items.filter(
              (f) => f.type === "file" && f.path.endsWith(".gguf") && f.path.toLowerCase().includes(quantLower),
            );
            if (ggufFiles.length > 0) {
              ...
              emitLog(SYSTEM_CONSOLE_ID, "info", `resolved file: ${resolvedFilename}`);
            } else {
                ...
                if (item.type === "directory" && item.path.toLowerCase().includes(quantLower)) {
                  ...
                  if (subResp.ok) {
                    ...
                    if (subGguf) {
                      ...
                      emitLog(SYSTEM_CONSOLE_ID, "info", `resolved file (subdir): ${resolvedFilename}`);
                      break;
                    }
                  }
                }
          ...
          emitLog(
            SYSTEM_CONSOLE_ID,
            "warn",
            `HF API lookup failed: ${apiErr instanceof Error ? apiErr.message : String(apiErr)}, using fallback`,
          );
        }
