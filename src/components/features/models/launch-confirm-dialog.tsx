      ...
      <DialogContent className="sm:max-w-[440px]">
        ...
        <Card className="gap-2 bg-muted/40 p-3 shadow-none text-xs">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="font-mono">{fmtBytes(model.sizeGb)}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-muted-foreground">Context length</span>
            <span className="font-mono">{fmtNum(model.contextLength)}</span>
          </div>
        </Card>
        <p className="text-[11px] text-muted-foreground">
          Switch to the Instances page and click <span className="font-medium">Launch instance</span> to start serving
          this model.
        </p>
        <DialogFooter>
          ...
          <Button onClick={() => onOpenChange(false)}>
            <CheckCircle2 className="mr-1.5 size-3.5" /> Got it
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
