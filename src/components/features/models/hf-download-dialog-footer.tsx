      ...
      <div className="mb-2 flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-muted-foreground">
        <span>
          ...
          <span className="font-mono font-semibold text-foreground">
            {selectedRepo ? `~${estimatedGb.toFixed(1)} GB` : "—"}
          </span>
        </span>
        {builder && (
          <>
            <span>·</span>
            <span>
              builder <span className="font-medium text-foreground">{builder}</span>
            </span>
          </>
        )}
        {filename && (
          <>
            <span>·</span>
            <span className="truncate font-mono">/models/{filename}</span>
          </>
        )}
      </div>
