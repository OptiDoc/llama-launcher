    ...
    <Section id="server" title="Server" open={true} onToggle={() => {}}>
      <div className="grid grid-cols-3 gap-3">
        <Field label="Port" error={errors.port}>
          <NumInput
            value={f.port}
            onChange={(v) => dispatch({ type: "SET_FIELD", key: "port", value: v })}
            error={errors.port}
          />
        </Field>
        <Field label="Host" error={errors.host}>
          <Input
            value={f.host}
            onChange={(e) => {
              dispatch({ type: "SET_FIELD", key: "host", value: e.target.value });
            }}
        ...
        <Field label="Parallel slots" error={errors.parallel}>
          <NumInput
            value={f.parallel}
            onChange={(v) => dispatch({ type: "SET_FIELD", key: "parallel", value: v })}
      ...
      <div className="grid grid-cols-3 gap-3">
        <Field label="Max predict (-1=∞)" error={errors.nPredict}>
          <NumInput
            value={f.nPredict}
            onChange={(v) => dispatch({ type: "SET_FIELD", key: "nPredict", value: v })}
            error={errors.nPredict}
          />
        </Field>
        <Field label="Timeout (sec)" error={errors.timeout}>
          <NumInput
            value={f.timeout}
            onChange={(v) => dispatch({ type: "SET_FIELD", key: "timeout", value: v })}
        ...
        <div className="grid gap-1.5">
          ...
          <div className="flex items-center gap-4 h-8">
            <label className="flex items-center gap-1.5 text-xs">
              <SW
                checked={f.contBatching}
                onCheckedChange={(v) => dispatch({ type: "SET_FIELD", key: "contBatching", value: v })}
              />
              Cont. batching
            </label>
            <label className="flex items-center gap-1.5 text-xs">
              <SW
                checked={f.metrics}
                onCheckedChange={(v) => dispatch({ type: "SET_FIELD", key: "metrics", value: v })}
              />
              Metrics
            </label>
          </div>
