    ...
    <Section id="advanced" title="Advanced" open={true} onToggle={() => {}}>
      ...
      <div className="grid grid-cols-3 gap-3">
        <Field label="Reasoning format">
          <Select
            value={f.reasoningFormat}
            onValueChange={(v) => dispatch({ type: "SET_FIELD", key: "reasoningFormat", value: v })}
          >
            <SelectTrigger className="w-full h-8 text-xs">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {["auto", "none", "deepseek", "deepseek-legacy"].map((t) => (
        ...
        <Field label="Reasoning budget (-1=∞)" error={errors.reasoningBudget}>
          <NumInput
            value={f.reasoningBudget}
            onChange={(v) => dispatch({ type: "SET_FIELD", key: "reasoningBudget", value: v })}
            error={errors.reasoningBudget}
          />
        </Field>
        <Field label="Log level (0-5)" error={errors.logLevel}>
          <NumInput
            value={f.logLevel}
            onChange={(v) => dispatch({ type: "SET_FIELD", key: "logLevel", value: v })}
      ...
      <div className="grid grid-cols-4 gap-3">
        <Field label="RoPE scaling">
          <Select
            value={f.ropeScaling}
            onValueChange={(v) => dispatch({ type: "SET_FIELD", key: "ropeScaling", value: v })}
          >
            <SelectTrigger className="w-full h-8 text-xs">
              <SelectValue placeholder="default" />
            </SelectTrigger>
            <SelectContent>
              {["", "none", "linear", "yarn"].map((t) => (
                <SelectItem key={t} value={t}>
                  {t || "default"}
                </SelectItem>
              ))}
        ...
        <Field label="RoPE scale" error={errors.ropeScale}>
          <NumInput
            value={f.ropeScale}
            onChange={(v) => dispatch({ type: "SET_FIELD", key: "ropeScale", value: v })}
            error={errors.ropeScale}
          />
        </Field>
        <Field label="RoPE freq base" error={errors.ropeFreqBase}>
          <NumInput
            value={f.ropeFreqBase}
            onChange={(v) => dispatch({ type: "SET_FIELD", key: "ropeFreqBase", value: v })}
            error={errors.ropeFreqBase}
          />
        </Field>
        <Field label="RoPE freq scale" error={errors.ropeFreqScale}>
          <NumInput
            value={f.ropeFreqScale}
            onChange={(v) => dispatch({ type: "SET_FIELD", key: "ropeFreqScale", value: v })}
            error={errors.ropeFreqScale}
          />
        </Field>
      </div>
