...
export function TopBarWindowControls({ maximized, onToggleMaximize }: TopBarWindowControlsProps) {
  const minimize = React.useCallback(async () => {
    ...
    try {
      ...
      log.error("[TopBar] minimize failed", {
        category: "ui",
  ...
  return (
    <div className="flex items-center">
      ...
      <button
        onClick={close}
        className="grid size-8 place-items-center rounded-lg text-muted-foreground hover:bg-red-500 hover:text-white transition-colors"
        aria-label="Close"
      >
