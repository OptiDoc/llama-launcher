...
export function ProfilesPage() {
  ...
  const visibleProfiles = React.useMemo(
    () => profiles.filter((p) => p.workspaceId === null || p.workspaceId === activeWorkspaceId),
    [profiles, activeWorkspaceId],
  );
  const globalProfiles = React.useMemo(() => visibleProfiles.filter((p) => p.scope === "global"), [visibleProfiles]);
  const modelProfiles = React.useMemo(() => visibleProfiles.filter((p) => p.scope === "model"), [visibleProfiles]);
  const sharedCount = React.useMemo(() => visibleProfiles.filter((p) => p.shared).length, [visibleProfiles]);

  const currentList = tab === "global" ? globalProfiles : tab === "model" ? modelProfiles : visibleProfiles;

  const selectedProfile = selectedId ? (visibleProfiles.find((p) => p.id === selectedId) ?? null) : null;
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-[12px] text-muted-foreground">
            Reusable llama-server argument presets · {visibleProfiles.length} total ({globalProfiles.length} global,{" "}
            {modelProfiles.length} model-bound, {sharedCount} shared)
          </p>
        </div>
