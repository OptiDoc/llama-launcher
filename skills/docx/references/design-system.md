...
function buildCoverR2(config) {
  ...
  children.push(
    new Paragraph({
      ...
      children: [
        new TextRun({ text: config.footerRight || "", size: 18, color: P.footerColor, font: { ascii: "Arial" } }),
      ],
    }),
  );
  ...
  if (config.subtitle) {
    children.push(
      new Paragraph({
        ...
        spacing: { after: 200 },
        children: [
          new TextRun({ text: config.subtitle, size: 30, font: { eastAsia: "SimSun", ascii: "Times New Roman" } }),
        ],
      }),
    );
  }
