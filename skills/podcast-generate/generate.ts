...
async function callZAI(systemPrompt: string, userPrompt: string, temperature: number): Promise<string> {
...
function validateScript(script: string, cfg: GenConfig, budgetLow: number, budgetHigh: number): [boolean, string[]] {
...
async function main() {
  ...
  for (let attempt = 1; attempt <= cfg.maxAttempts; attempt++) {
    const [systemPrompt, userPrompt] = buildPrompts(material, cfg, durationMin, target, low, high, attemptHint);
