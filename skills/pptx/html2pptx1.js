...
function extractTextContent(el) {
  ...
  if (Array.isArray(el.rows)) {
    return el.rows
      .map((row) => row.map((cell) => (typeof cell === "string" ? cell : cell?.text || "")).join(" "))
      .join(" ");
  }
  return "";
}
...
function checkElementBounds(slideData, sw, sh) {
  ...
  for (const el of slideData.elements) {
    ...
    if (p.y + p.h > sh + tol)
      w.push(`⚠ BOUNDS: "${getElementLabel(el)}" extends ${((p.y + p.h - sh) * 72).toFixed(0)}pt beyond BOTTOM`);
  }
  return w;
}
...
function calculateWidthCompensation(el, slideWidthIn) {
  ...
  const isItalic = el.style?.italic === true || (Array.isArray(el.text) && el.text.some((r) => r.options?.italic));
  ...
  if (single && el.position.w > 1.5 && txt.length >= 14 && !el.noWrap && !hasLetterSpacing && !isItalic) {
    f = Math.min(f, 0.05);
  }
...
function shouldUseNumericEmphasisText(text = "") {
  ...
  const labelPrefix =
    /^(?:lv|no|honor|resource|step|part|section|chapter|page|slide|p|q|a|item|case|fig|table)\b/i.test(compact);
    ...
    tasks.push(
      (async () => {
        try {
          ...
          await sharp(svgBuffer, { density: 192 }).resize(widthPx, heightPx, { fit: "fill" }).png().toFile(outPath);
          el.src = outPath;
        } catch (err) {
  ...
  const tasks = slideData.elements.map(async (el) => {
    ...
    try {
      ...
      if (el.objectFit === "cover") {
        ...
        const resized = await sharp(input).rotate().resize(resizedW, resizedH, { fit: "fill" }).png().toBuffer();
        await sharp({
          create: {
            width: targetW,
            height: targetH,
  ...
  for (const el of sortedElements) {
    if (el.type === "image") {
      ...
      try {
        ...
        const isLocalFile = typeof imagePath === "string" && !isDataUrl && !/^https?:\/\//i.test(imagePath);
      ...
      let adjustedH = el.position.h * (1 + COMPENSATION.LIST_HEIGHT) + getListSpacingHeightBonus(el);
      ...
      if (clip) {
        const clipRight = clip.x + clip.w + getNarrowBodyTextClipSlack(el) + getMediumSingleLineClipSlack(el);
        if (adjustedX < clip.x) {
      ...
      // These run after the box width is finalized so we know the real frame.
      {
        ...
        const _oneLine = _fsPt > 0 && !hasExplicitTextBreaks(el) && (Number(el.position?.h) || 0) * 72 <= _linePt * 1.8;
...
async function extractSlideData(page, slideDims) {
  return await page.evaluate((slideDims) => {
    ...
    const appendTextRuns = (runs, text, baseOptions = {}) => {
      ...
      const shouldSplit =
        (hasCJKChars(fullText) && (hasLatinChars(fullText) || hasDigitChars(fullText) || hasEmojiChars(fullText))) ||
    ...
    const computeTransparency = (colorStr, opacityStr) => {
      ...
      const opacityT = Number.isFinite(cssOpacity) && cssOpacity < 1 ? Math.round((1 - cssOpacity) * 100) : null;
    ...
    const hasTextBackgroundClip = (computed) => {
      const clip = [computed.backgroundClip, computed.webkitBackgroundClip, computed.WebkitBackgroundClip]
        .filter(Boolean)
        .join(" ")
        .toLowerCase();
    ...
    const resolveTextPaintStyle = (computed, element = null) => {
      const fillColor =
        computed.webkitTextFillColor && computed.webkitTextFillColor !== "currentcolor"
          ...
          : computed.color;
      const transparentTextPaint = isFullyTransparentCssColor(fillColor) || isFullyTransparentCssColor(computed.color);
    ...
    const renderLinearGradientBackground = (bgImage, widthPx, heightPx) => {
      ...
      for (const layer of layers.reverse()) {
        if (layer.type === "radial") {
          ...
          const len =
            Math.abs(canvas.width * vx) + Math.abs(canvas.height * vy) || Math.max(canvas.width, canvas.height);
    ...
    document.querySelectorAll("h1,h2,h3,h4,h5,h6,p").forEach((parent) => {
      ...
      const blockTextChildren = children.filter(
        (c) =>
          ["SPAN", "B", "STRONG", "I", "EM", "U", "A", "LABEL", "CODE", "MARK", "TIME", "CITE", "ABBR", "S"].includes(
            c.tagName,
          ) &&
          c.textContent.trim() &&
          _isBlockDisplay(c),
    ...
    document.querySelectorAll("*").forEach((parent) => {
      ...
      const blockTextChildren = Array.from(parent.children).filter(
        (c) => INLINE_TEXT_CHILD_TAGS.includes(c.tagName) && c.textContent.trim() && _isBlockDisplay(c),
      );
    ...
    document.querySelectorAll("*").forEach((parent) => {
      ...
      const allInline = textKids.every((c) => INLINE_TEXT_CHILD_TAGS.includes(c.tagName) && !_isBlockDisplay(c));
      if (!allInline) return;
    ...
    const getNearestTextClipBounds = (el) => {
      ...
      while (p && p !== document.body && p !== document.documentElement) {
        ...
        const borders = [cs.borderTopWidth, cs.borderRightWidth, cs.borderBottomWidth, cs.borderLeftWidth].map(
          (v) => parseFloat(v) || 0,
        );
    ...
    const getFlexTextRightBound = (el, rect) => {
      ...
      const rightPx = nextRightSibling ? nextRightSibling.left - Math.max(2, gap * 0.5) : pRect.right - padR;
    ...
    document.querySelectorAll("*").forEach((el) => {
      ...
      if (el.tagName === "IMG") {
        ...
        if (rect.width > 0 && rect.height > 0) {
          ...
          if (br > 0 && computed.borderRadius.includes("%") && br >= 50 && Math.abs(rect.width - rect.height) < 2) {
            rounding = true;
          }
          ...
          for (const tr of trList) {
            ...
            const rowOut = cells.map((cell) => {
              ...
              const cellOpts = {
                ...
                align:
                  cellAlign === "left" || cellAlign === "right" || cellAlign === "center" || cellAlign === "justify"
                    ? cellAlign
                    : "left",
      ...
      const isStandaloneInlineShape = inlineAsBlockTags.has(el.tagName) && !el.closest("p,h1,h2,h3,h4,h5,h6,li,ul,ol");
      ...
      if (isContainer) {
        ...
        let hasBg =
          !isTextClippedBackground && computed.backgroundColor && computed.backgroundColor !== "rgba(0, 0, 0, 0)";
          ...
          if (isLinear && !gradientDataUrl) {
            ...
            if (colorTokens.length >= 2) {
              ...
              if (!opaqueStop || mixedAlpha) {
                ...
                gradientTransparency = Math.round(stops.reduce((acc, s) => acc + s.t, 0) / stops.length);
              }
            }
        ...
        const borderBoxTriangle =
          Math.abs(cssBoxW - (borders[1] + borders[3])) <= 1 && Math.abs(cssBoxH - (borders[0] + borders[2])) <= 1;
          ...
          if (rect.width > 0 && rect.height > 0) {
            ...
            if (hasBg || hasUniformBorder) {
              ...
              if (gradientFill) {
                ...
                fillOut = bgFillValue.startsWith("#") ? bgFillValue.substring(1).toUpperCase() : rgbToHex(bgFillValue);
      ...
      if (el.tagName === "UL" || el.tagName === "OL") {
        ...
        if (!isOrdered && liElements.length > 0) {
          ...
          const listStyleNone =
            (liStyle.listStyleType === "none" && ulStyle.listStyleType === "none") || liStyle.listStyleType === "none";
          if (listStyleNone) {
        ...
        liElements.forEach((li, idx) => {
          ...
          if (runs.length > 0) {
            ...
            if (customBullet) {
              ...
              runs[0].options.bullet = isOrdered ? { type: "number", indent: textIndent } : { indent: textIndent };
            }
          }
      ...
      const noWrap =
        computed.whiteSpace === "nowrap" || computed.whiteSpace === "pre" || isRowFlexText || isFlexRowTitleText;
      ...
      const parseTextShadow = (ts) => {
        ...
        const offset = Math.sqrt(offsetX * offsetX + offsetY * offsetY) * PT_PER_PX;
        const angle = offsetX === 0 && offsetY === 0 ? 0 : ((Math.atan2(offsetY, offsetX) * 180) / Math.PI + 360) % 360;
        return {
          type: "outer",
      ...
      const pushInlineBackgroundShapes = () => {
        ...
        for (const node of inlineNodes) {
          ...
          if (radiusValue > 0) {
            if (radius.includes("%"))
              rectRadius = (Math.min(nodeRect.width, nodeRect.height) * radiusValue) / 100 / PX_PER_IN;
      ...
      if (textIsIconOnly && baseStyle._centerInFrame && rotation === null) {
        ...
        const lineHeightPx =
          computed.lineHeight && computed.lineHeight !== "normal" ? parseFloat(computed.lineHeight) : fsPx * 1.2;
      ...
      const hasFormatting =
        !el._directTextOnly &&
        el.querySelector("b, i, u, strong, em, span, br, a, sub, sup, s, del, strike, mark, code, small, big");
    ...
    try {
      ...
      try {
        await page.evaluate(() => {
          ...
          return Promise.race([document.fonts.ready, new Promise((resolve) => setTimeout(resolve, 3000))]);
        });
      } catch (e) {
        /* ignore */
      }
